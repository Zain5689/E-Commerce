  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 min-h-[75vh]">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-200 font-semibold">{t.accountPageTitle}</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all border border-slate-700 bg-slate-900 hover:bg-slate-800 text-brand-400 shadow-sm"
          >
      ...
      {!isLoggedIn ? (
        ...
        <div className="max-w-md mx-auto bg-[#131b2e] border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          {/* Tab Switcher */}
          <div className="flex bg-slate-900/90 border border-slate-800 rounded-2xl p-1">
            <button
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors"
                  />
                  <Mail className={`w-4 h-4 text-slate-500 absolute ${isArabic ? 'left-3' : 'right-3'} top-1/2 -translate-y-1/2`} />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`absolute ${isArabic ? 'left-3' : 'right-3'} top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300`}
                  >
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors font-mono"
                />
              </div>
              ...
              <div className="flex items-start gap-2 pt-1">
                <input
                  ...
                  id="agreeTerms"
                  defaultChecked
                  className="w-4 h-4 mt-0.5 rounded border-slate-700 bg-slate-800 text-brand-600 focus:ring-brand-500"
                />
                <label htmlFor="agreeTerms" className="text-[11px] text-slate-400 select-none cursor-pointer leading-tight">
                  {t.agreeTerms}
                </label>
              </div>
          ...
          <div className="space-y-3 pt-2 border-t border-slate-800 text-center">
            <p className="text-[11px] text-slate-400 uppercase tracking-wider">{t.orContinueWith}</p>
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setIsLoggedIn(true)}
                className="flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 py-2.5 rounded-xl text-xs font-semibold text-white transition-colors"
              >
                <span>Google</span>
              </button>
              <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          {/* Sidebar Navigation */}
          <div className="lg:col-span-1 bg-[#131b2e] border border-slate-800 rounded-3xl p-5 space-y-6 shadow-xl">
            {/* User Mini Profile */}
            <div className="flex items-center gap-3.5 pb-4 border-b border-slate-800">
              ...
              <div className="min-w-0">
                ...
                <span className="text-[10px] text-brand-400 font-bold bg-brand-500/10 border border-brand-500/20 px-2 py-0.5 rounded-full inline-block mt-0.5">
                  VIP Member
                </span>
              </div>
            </div>
            ...
            <nav className="space-y-1.5 text-xs font-bold">
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-rose-400 hover:bg-rose-950/30 transition-colors pt-4 border-t border-slate-800"
              >
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-[#131b2e] border border-slate-800 rounded-2xl p-4.5 space-y-1 hover:border-slate-700 transition-colors">
                <span className="text-xs text-slate-400 font-medium">{t.totalSpent}</span>
              ...
              <div className="bg-[#131b2e] border border-slate-800 rounded-2xl p-4.5 space-y-1 hover:border-slate-700 transition-colors">
                <span className="text-xs text-slate-400 font-medium">{t.activeOrdersCount}</span>
                <div className="text-xl font-black text-amber-400 font-mono">
                  {MOCK_USER.activeOrders} {isArabic ? 'طلبات' : 'Orders'}
                </div>
              </div>
              <div className="bg-[#131b2e] border border-slate-800 rounded-2xl p-4.5 space-y-1 hover:border-slate-700 transition-colors">
                <span className="text-xs text-slate-400 font-medium">{t.completedOrdersCount}</span>
            ...
            {(activeTab === 'overview' || activeTab === 'orders') && (
              <div className="bg-[#131b2e] border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-5 shadow-xl">
                ...
                <div className="space-y-4">
                  {MOCK_ORDERS.map((order) => (
                    <div
                      key={order.id}
                      className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-5 space-y-4 hover:border-slate-700 transition-colors"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {order.status === 'shipped' && (
                            <span className="bg-sky-500/15 text-sky-400 border border-sky-500/30 text-[11px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                      ...
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                          ...
                          {order.items.map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                              ...
                              <div className="text-xs">
                                <p className="font-bold text-slate-200 line-clamp-1 max-w-[180px] sm:max-w-xs">{item.name}</p>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            className="bg-brand-600/90 hover:bg-brand-500 text-white font-bold px-3.5 py-2 rounded-xl text-xs transition-all shadow active:scale-95 flex items-center gap-1.5"
                          >
              ...
              <div className="bg-[#131b2e] border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-5 shadow-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {MOCK_ADDRESSES.map((addr) => (
                    <div
                      key={addr.id}
                      className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4.5 space-y-3 relative hover:border-slate-700 transition-colors"
                    >
                      <div className="flex items-center justify-between">
                      ...
                      <div className="space-y-1 text-xs text-slate-300">
                        ...
                        <p className="text-slate-400">{isArabic ? addr.cityAr : addr.city}</p>
                        <p className="text-slate-400">{addr.address}</p>
                        <p className="text-brand-400 font-mono pt-1">{addr.phone}</p>
                      </div>

                      <div className="flex items-center gap-3 pt-2 border-t border-slate-800 text-xs">
                        <button className="text-slate-400 hover:text-brand-400 flex items-center gap-1 transition-colors">
                          <Edit2 className="w-3 h-3" /> {t.editAddress}
                        </button>
                        <button className="text-slate-400 hover:text-rose-400 flex items-center gap-1 transition-colors">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                <div className="bg-[#131b2e] border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4 shadow-xl">
                  <h2 className="text-base sm:text-lg font-black text-white">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-slate-300">{t.emailAddress}</label>
                      <input
                        type="email"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-slate-300">{t.phoneNumber}</label>
                      <input
                        type="tel"
                ...
                <div className="bg-[#131b2e] border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4 shadow-xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-slate-300">{t.currentPassword}</label>
                      <input
                        type="password"
                        placeholder="••••••••"
                        className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-brand-500"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-slate-300">{t.newPassword}</label>
                      <input
                        type="password"
                        placeholder="••••••••"
                        className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-brand-500"
                  ...
                  <button
                    ...
                    className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs transition-all border border-slate-700 shadow mt-2"
                  >
                    {t.updatePasswordBtn}
                  </button>
                </div>
      ...
      {trackingOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-6 shadow-2xl relative">
            <button
              ...
              {[
                ...
                <div key={idx} className="flex gap-3.5">
                  <div className="flex flex-col items-center">
                    ...
                    {idx < 3 && <div className={`w-0.5 h-8 my-0.5 ${step.done ? 'bg-emerald-500/50' : 'bg-slate-800'}`} />}
                  </div>
                  <div>
                    <h4 className={`text-xs font-bold ${step.done ? 'text-white' : 'text-slate-500'}`}>{step.title}</h4>
